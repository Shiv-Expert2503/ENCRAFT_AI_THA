...
# def extract_and_crop_drawings(pdf_path, output_dir, dpi=300):
#     """
#     Extracts high-res images, forcefully removes the title block, 
...
#         # We slice off the right 16% of the page to eliminate the flag notes.
#         # (Adjust the 0.84 factor slightly if it cuts too much/too little)
#         # ---------------------------------------------------------
...
#         # ---------------------------------------------------------
#         # 3. MORPHOLOGICAL DILATION: Smear the ink into blobs
#         # Dynamically size the kernel based on image width (approx 2%).
#         # This bridges the gaps between text and ducts within a single drawing,
#         # but leaves the massive white gap between the top and bottom drawings intact.
#         # ---------------------------------------------------------
#         k_size = int(pix.w * 0.02) 
#         kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (k_size, k_size))
#         dilated = cv2.dilate(thresh, kernel, iterations=1)
        
#         # 4. Find Contours on the "smeared" blobs
#         contours, _ = cv2.findContours(dilated, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        
#         # Filter for large blobs (at least 5% of the total cropped area) to ignore random specs
#         min_area = (crop_w * pix.h) * 0.05
#         valid_contours = [c for c in contours if cv2.contourArea(c) > min_area]
        
#         # Sort top to bottom so drawing 1 is top, drawing 2 is bottom
#         valid_contours = sorted(valid_contours, key=lambda c: cv2.boundingRect(c)[1])
        
#         if not valid_contours:
#             print(f"No clear drawings found on {base_name} Page {page_num + 1}. Saving cropped page.")
#             out_path = os.path.join(output_dir, f"{base_name}_page{page_num+1}_fallback.png")
#             cv2.imwrite(out_path, main_area)
#             continue

#         # 5. Crop the ORIGINAL crisp image using the blob boundaries
#         for i, contour in enumerate(valid_contours):
#             x, y, w, h = cv2.boundingRect(contour)
            
#             # Add a small 1% padding margin around the drawing so lines don't get clipped
#             pad = int(pix.w * 0.01)
#             x1 = max(0, x - pad)
#             y1 = max(0, y - pad)
...
#             out_filename = f"{base_name}_page{page_num+1}_drawing{i+1}.png"
...
#     pdf_file = "/home/rareboy/PycharmProjects/encraft_ai/data/image_2.pdf" 
#     output_directory = "./extracted_drawings_v2_again"
    

    
#     extract_and_crop_drawings(pdf_file, output_directory)
